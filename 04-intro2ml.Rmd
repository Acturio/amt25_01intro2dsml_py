...
Una forma ligeramente más compleja pero que también permite una mejor estrucura de programación, escalabilidad y control de evolutivos, es hacer el proceso anterior usando Programación Orientada a Objetos (POO). El ejemplo quedaría programado de la siguiente forma:
...
class RatioTransformer(BaseEstimator, TransformerMixin):
    def __init__(self, numerator, denominator, new_feature_name):
    ...
    def fit(self, X, y=None):
        self.feature_names_out_ = list(X.columns) + [self.new_feature_name]
        return self

    def transform(self, X):
        X = X.copy()
        X[self.new_feature_name] = X[self.numerator] / X[self.denominator]
        return X
...
column_transformer = ColumnTransformer(
    transformers=[
        ('ratio', RatioTransformer('Column1', 'Column2', 'c1_over_c2'),
         ['Column1', 'Column2'])
    ],
    remainder='passthrough',
    verbose_feature_names_out=False
...
Esta transformación permite eliminar categorías particulares de una variable categórica. Esto es útil porque no siempre todas las categorías son relevantes y al momento de usar *OneHotEncoder*, la dimensión de variables crece y no necesariamente todas las categorías aportan valor, por lo que sería mejor eliminarlas.
...
class DropColumnsTransformer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        cols = [c for c in X_.columns if c not in self.columns_to_drop]
        return X_[cols]
    
    def set_output(self, *, transform=None):
        # para permitir set_output en ColumnTransformer
        return self
       
    def get_feature_names_out(self, input_features=None):
        if input_features is None:
            return []
        return [c for c in input_features if c not in self.columns_to_drop]


```


Una transformación particular que resulta muy interesante y útil, es la recodificación de categorías. Esta transformación permite que el usuario pueda colapsar categorías originales en otras que resulten más útiles o valiosas para la predicción.

```{python}
class RecodeCategoryTransformer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        for col, map_dict in self.mapping.items():
            X[col] = X[col].map(map_dict).fillna(X[col])
        return X
...
Aplicando ambos ejemplos a un dataset dummy, se obtiene:

```{python}
df = pd.DataFrame({
    'Color': ['rojo','azul','verde','rojo','azul'],
    'Talla': ['S','M','L','XL','S'],
    'Precio': [10,20,15,30,22]
})
...
column_transformer = ColumnTransformer(
    transformers=[
        # Paso 1 + 2: Recode + OneHotEncoder
        ('color_pipeline',
         Pipeline([
             ('recode', RecodeCategoryTransformer(recode_map)),
             ('ohe', OneHotEncoder(sparse_output=False))#,
#             ('drop', DropColumnsTransformer(['Color_secundario']))
         ]),
         ['Color']
        ),

        # Talla con OHE simple
        ('talla_ohe', 
        Pipeline([
         ('ohe2', OneHotEncoder(sparse_output=False)),
         ('drop', DropColumnsTransformer(['Talla_S', 'Talla_XL']))
         ]),
         ['Talla']
        )
    ],

    remainder='passthrough',     # deja Precio igual
    verbose_feature_names_out=False
).set_output(transform='pandas')
...
df_test = pd.DataFrame({
    'Color': ['azul','verde','rojo','verde'],
    'Talla': ['M','XL','S','L'],
    'Precio': [50,40,45,38]
})

column_transformer.transform(df_test)
```

En este ejemplo se muestra cómo recodificar las categorías del color, creando un pipeline particular para la variable *Color* y paralelemante otro pipeline se encarga de dicotomizar las categorías del color y elegir manualmente cuáles son las categorías a eliminar.
