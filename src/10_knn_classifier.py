...
(
  summary_df >>
  ggplot(aes(x = "param_n_neighbors", y = "mean_test_roc_auc", size = "param_p",
             color = "param_metric", shape = "param_weights")) +
  geom_point(alpha = 0.65) +
  ggtitle("Parametrización de KNN vs ROC AUC") +
...
(
  ...
  mutate(
    ymin = np.maximum(0, _.mean_test_roc_auc - _.std_test_roc_auc),
    ymax = np.minimum(1, _.mean_test_roc_auc + _.std_test_roc_auc)) >>
  ggplot(aes(x = "param_n_neighbors", y = "mean_test_roc_auc")) +
  geom_errorbar(aes(ymin='ymin', ymax='ymax'),
    width=0.3, position=position_dodge(0.9)) +
  geom_point(alpha = 0.65) +
  ggtitle("Parametrización de KNN vs ROC") +
  xlab("Parámetro: Número de vecinos cercanos") +
  ylab("ROC promedio")
)
...
# Entrenar el pipeline
final_knn_pipeline.fit(telco_train_selected, telco_y_train)

## Predicciones finales
y_pred_knn = final_knn_pipeline.predict_proba(telco_x_test)[:,0]
Churn_Pred = np.where(y_pred_knn >= 0.7, "No", "Yes")

results = (
  telco_x_test >>
  mutate(
    Churn_Prob = y_pred_knn, 
    Churn_Pred = Churn_Pred,
    Churn = telco_y_test) >>
  select(_.Churn_Prob, _.Churn_Pred, _.Churn)
)
...
for i in range(telco_x_test[columnas_seleccionadas].shape[1]):
    ...
    y_pred_permuted = final_knn_pipeline.predict_proba(telco_x_test_permuted)[:,0]
