...
```{r, echo = F, fig.align = "center", fig.cap= "Páneles de trabajo de Positron"}
knitr::include_graphics("img/02-intro2py/02_rstudio_py.png")
```

## Uso de python en Positron

### Instalar ambiente virtual desde bash

El primer paso es instalar la librería virtualenv, la cual permite crear múltiples ambientes virtuales
```{bash, eval=F}
pip install virtualenv
virtualenv --version
```

El segundo paso es crear el ambiente virtual seleccionando la versión de python que se desea usar
```{bash, eval=F}
virtualenv -p python3.11.8 nombre_ambiente
```

Una vez creado el ambiente, se carga el ambiente.

**Windows (Powershell)**
```{bash, eval=F}
Set-ExecutionPolicy -ExecutionPolicy RemoteSigned -Scope CurrentUser 
Y
.\nombre_ambiente\Scripts\Activate.ps1
```
...
Es importante estar situado en la carpeta del proyecto. Esto permite acceder al ambiente con los comandos anteriores.
El último paso es instalar las librerías desde un archivo .txt:

```{bash, eval=F}
pip install -r requirements.txt
```

y listo! Podemos comenzar a trabajar

Para desacrtivar el ambiente, se usa:

```{bash, eval=F}
deactivate
```

### Instalar ambiente virtual desde R
